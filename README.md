...
Time-bearing values (`DateTime`, `Time` and the boundaries of their intervals) may carry the [`time-precision`](http://hl7.org/fhir/StructureDefinition/time-precision) extension on the primitive element (`_valueDateTime`, `_valueTime`, `_start`, `_end`). FHIR `dateTime` and `time` require minutes and seconds once an hour is present, while CQL allows hour- and minute-precision values, so a conformant engine zero-pads the missing components and declares the real precision with that extension (`h`, `min`, `s` or `ms`). The runner drops the padded components again before comparing, so a `valueDateTime` of `2005-05-10T10:00:00Z` declared at `h` is read as `@2005-05-10T10Z`; any timezone offset is kept as sent.
