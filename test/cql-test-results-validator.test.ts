...
describe('CQLTestResults.toJSON actual serialization', () => {
	...
	it('keeps JSON rendering for four-key objects whose closed flags are not booleans', () => {
		const results = new CQLTestResults(new CQLEngine('http://localhost:8080/fhir/$cql'));
		results.add({
			testsName: 'T',
			groupName: 'G',
			testName: 'NearMissTuple',
			expression: 'e',
			testStatus: 'pass',
			invalid: 'false',
			actual: { lowClosed: 'true', low: 1, highClosed: 'false', high: 4 },
			expected: 'Tuple { lowClosed: true, low: 1, highClosed: false, high: 4 }',
		} as any);

		const json = results.toJSON();
		expect(json.results[0].actual).toBe(
