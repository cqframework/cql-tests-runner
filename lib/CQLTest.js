...
class CQLTest {
    ...
    _prepareTest() {
        ...
        this.config.id = `${this.config.type}_${this.config.group}_${this.config.name}`;
    ...
    async run(apiUrl) {
        if (this.prepared) {
            ...
            try {
                console.log('Running test %s:%s:%s', this.config.type, this.config.group, this.config.name);
                ...
                console.log('Test %s:%s:%s status: %s expected: %s actual: %s', this.config.type, this.config.group, this.config.name, result.testStatus, this.config.expected, result.actual);
            }
            return result;
        } else {
